...
We periodically run a **dual-model, fresh-context adversarial review** to hunt for bugs that quietly lose money — especially before any live-money change. First run: 2026-06-16 on the Python bot (produced [docs/superpowers/plans/2026-06-16-bot-money-loss-remediation.md](docs/superpowers/plans/2026-06-16-bot-money-loss-remediation.md), 34 findings). **The next planned run is the same process on the `dashboard/` subproject, once the bot Phase-1 fixes are live.** How to do it:
