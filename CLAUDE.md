...
Alpaca trading sandbox running scheduled bots on **GitHub Actions cron** across **two accounts in parallel** — a "manual" paper account where the user opens trades by hand and the bot manages them (trail/ladder/stop on every stock you hold, 50% close on existing puts, covered call sale on assignment) but never opens new puts itself, and a "live" REAL-MONEY account that behaves identically to manual on separate Alpaca live credentials. Notifications flow to a private Discord server (separate channels per account); structured logs are committed back to this repo as JSONL.

> **Account sunset executed 2026-06-29.** The conservative + aggressive paper wheels and the three small-account auto-spread accounts (`sm500`/`sm1000`/`sm2000`) were removed — config rows, workflows, cron jobs, Discord channels, the conservative-only congress-copy bot, the daily-summary head-to-head, state/log files, and the dashboard enumerations. **The shared strategy engine is unchanged** — `strategy.py`, `wheel_strategy.py` (including the whole auto-spread engine, still flag-gated), `long_options_strategy.py`, and `screener_core.py` all stay intact; manual + live keep every behavior they had. Much of the multi-account history below is retained for context but describes accounts that no longer run. **Loose ends to finish by hand:** delete the retired cron-job.org jobs (the setup script only patches/creates, never deletes) and remove the now-unused `ALPACA_*`/`DISCORD_*` GitHub Actions + Vercel secrets for the five gone accounts.
...
- **Account sunset — EXECUTED 2026-06-29** (was planned for 2026-07-15; pulled forward at Tim's request). Removed `conservative`, `aggressive`, and the three small accounts (`sm500`/`sm1000`/`sm2000`), narrowing the system to `manual` + `live`. Touch-points completed: `config.py → MODES`, the `tsla-monitor-*` + `wheel-screener-*` workflows for those accounts, `congress-copy` (conservative-only), the `daily-summary` steps + head-to-head, `tools/setup_cronjobs.py`, state/log files, `reconcile_agg_state.py`, the read-only skill tools, and the dashboard account enumerations + tests. **Still to do by hand (out of code scope):** delete the retired cron-job.org jobs (the setup script PATCHes/creates only, never deletes — until deleted they dispatch to now-missing workflows and 404 harmlessly), and remove the unused `ALPACA_*` / `DISCORD_*` GitHub Actions + Vercel secrets for the five gone accounts.
...
- **Live account: deposit/withdrawal detection — ✅ SHIPPED 2026-06-30.** `account_funding.py` reads Alpaca's `/v2/account/activities` (`CSD`/`CSW` records — the authoritative transfer log, **not** the equity-diff originally sketched here, which would false-positive on trading P&L and miss small deposits) each live cycle and posts a 🟢 deposit / 🔴 withdrawal embed to `#live-trades` with the exact amount + new balance. Dedup + first-run silent seed via committed `account_state_live.json` (so historical/initial funding never pings); runs as a `continue-on-error` step in `tsla-monitor-live.yml`; fully fail-soft. The 4:12 PM live summary carries a "💵 Funding Today" line (`daily_summary._funding_field`), and the dashboard live card has a funding panel + a "Deposit funds ↗" deep-link to `app.alpaca.markets/brokerage/funding/deposit/ach`. Note: the Trading API can't *initiate* transfers (Broker-API-only) — Tim still deposits on Alpaca's site; this just makes the system aware. Live only (mode-parameterized; paper is a one-line add). Spec/plan: `docs/superpowers/{specs,plans}/2026-06-30-live-funding-detection*`. Tests: 602 pytest / 822 vitest.
...
**From a phone (no laptop) — `deploy-dashboard.yml`, added 2026-08-17.** A GitHub Actions workflow runs the Vercel CLI on GitHub's runners. Trigger it either by pushing to `main` touching `dashboard/**`, or from the Actions tab → "Deploy Dashboard" → "Run workflow" (works in the GitHub mobile app). **Deploy target follows the ref:** `main` → production, any other ref → a preview deployment (so the pipeline can be exercised end-to-end without touching the live site). The path filter keeps the bots' every-10-minute state/log commits from triggering deploys.

> **Why it uploads source instead of building on the runner.** The first cut used `vercel build` + `vercel deploy --prebuilt`, so the tested artifact was the shipped artifact. That can't work here: `dashboard/api/fundamentals.py` pulls yfinance → pandas → numpy, and off-Vercel the Python builder can't resolve wheels for Vercel's runtime — it force-bundles numpy and pandas from source and the function hits **276 MB against a 225 MB limit**. On Vercel's own builders the wheels resolve and it fits. So the workflow uploads source and lets Vercel build, byte-for-byte the same path as `npx vercel --prod` from a laptop. Trade-off: the deployed artifact is built on Vercel rather than being the one the tests ran against; the test gate still decides whether the commit deploys at all.

The full vitest suite + `tsc -b` gate the deploy — that gate is the point, since you can't run the suite locally from a phone; a `workflow_dispatch` checkbox (`skip_tests`) bypasses it for an urgent fix. The run summary prints the deployment URL.

Needs three GitHub Actions secrets: `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`. `.vercel/` is gitignored so the IDs can't be read from the repo — they must be secrets. ⚠️ **`VERCEL_TOKEN` must be scoped to the team ("Tim's projects"), not to a single project.** A project-scoped token authenticates against the REST API fine (it fetched the project with HTTP 200 during setup) but the CLI's own `/v2/user` lookup 404s on it, so every command dies with the misleading `Could not retrieve Project Settings`. Cost of the wider scope: the token can reach the other projects in the team. The dashboard's own env vars (ALPACA_*, KV, ANTHROPIC_API_KEY, …) stay in Vercel and are applied there when it builds and runs the deployment; nothing about runtime config moved.
