...
- **Account sunset plan (decided 2026-06-23)** — **Sunset/remove the three small accounts (`sm500`/`sm1000`/`sm2000`) on 2026-07-15.** They bled through the validation window (see the SM PCS hardening history above) and are now excluded from AI grading (2026-06-23, grading is manual+live only). Once they're gone, **consider removing the conservative and aggressive accounts as well**, narrowing the system's focus to `manual` + `live`. Nothing has been removed yet — this is the recorded intent; revisit on/after 2026-07-15. Removal touch-points when we execute: `config.py → MODES`, the `tsla-monitor-sm*` + `wheel-screener-*` workflows, cron-job.org schedules (`tools/setup_cronjobs.py`), the `daily-summary` steps, Discord channels/webhooks, GitHub Actions + Vercel secrets, and the dashboard account enumerations (`useAccount`/`account-utils`/`trade-types`/`alpaca.ts`).
