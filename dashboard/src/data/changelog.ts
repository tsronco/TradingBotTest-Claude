...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    details:
      'On a multi-leg options position (a spread, a covered call) the only thing telling you which leg you '
      + 'BOUGHT versus which you SOLD was the sign of the quantity — a "1" versus a "-1", buried in a number '
      + 'column. Easy to miss, and the difference matters (the long leg caps your risk; the short leg is what '
      + 'you collected premium on).\n\n'
      + 'Every option row now carries an explicit green LONG (you bought it) or red SHORT (you sold/wrote it) '
      + 'badge next to the symbol; short stock positions get the SHORT badge too. The qty column now shows an '
      + 'explicit + or − sign and turns red when negative, and a one-line legend at the top of the page spells '
      + 'out what each badge means. Display-only — no data or API change.',
  },
  {
    date: '2026-09-04',
    category: 'config',
  ...
  },
  {
    ...
    details:
      ...
      + 'New .github/workflows/deploy-dashboard.yml runs the Vercel CLI on GitHub\'s runners. Two ways '
      + 'to ship from a phone: push to main touching dashboard/**, or tap Run workflow in the Actions '
      + 'tab. Deploy target follows the ref — main goes to production, any other ref gets a preview '
      + 'deployment, so the pipeline can be exercised without touching the live site.\n\n'
      + 'It uploads source and lets Vercel build, rather than building on the runner. Building locally '
      + 'was the original design (so the tested artifact is the shipped one), but api/fundamentals.py '
      + 'pulls yfinance -> pandas -> numpy, and off-Vercel those cannot resolve wheels for Vercel\'s '
      + 'runtime: they force-bundle from source and the function hits 276 MB against a 225 MB limit.\n\n'
